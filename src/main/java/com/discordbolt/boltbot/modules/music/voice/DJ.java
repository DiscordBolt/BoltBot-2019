package com.discordbolt.boltbot.modules.music.voice;
...
import com.discordbolt.boltbot.modules.music.voice.internal.AudioProvider;
...
public class DJ extends AudioEventAdapter {
    ...
    public DJ(IGuild guild, AudioPlayer player) {
        ...
        MusicModule.getClient().getDispatcher().registerListener(this);
        guild.getAudioManager().setAudioProvider(new AudioProvider(player));
        setVolume(20);

        announceChannel = guild.getChannelsByName(MusicModule.TEXT_CHANNEL).stream().findAny().orElse(null);
    }
    ...
    public void skipCurrentTrack(int count) {
        if (playingRandom && getQueue().size() == 0) {
            try {
                MusicModule.getVoiceManager().queue(getGuild(), getRandomRequester(), MusicModule.getRandomSong());
    ...
    public void pause() {
        ...
        MusicModule.getClient().mute(getGuild(), true);
    }

    public void unpause() {
        player.setPaused(false);
        MusicModule.getClient().mute(getGuild(), false);
    }
    ...
    public void onTrackStart(AudioPlayer player, AudioTrack track) {
        ...
        nowPlayingMessage = ChannelUtil.sendMessage(announceChannel, MusicModule.createPlayingEmbed(getGuild(), track));
    ...
    private void checkChannel() {
        if (getVoiceChannel().getConnectedUsers().size() <= 1) {
            MusicModule.getVoiceManager().forceLeaveChannel(guild);
        }
    }
